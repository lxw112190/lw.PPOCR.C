...
int main(void) {
    ...
    status = lw_scalar_squeeze_f32(
        tensor_input, reshape_output, 4u, squeeze_input_dimensions,
        2u, squeeze_axes, 2u, squeeze_output_dimensions);
    ...
    status = lw_scalar_reduce_mean_f32(
        ...
        2u, reduce_axes, 1u, 0u, 3u, reduce_output_dimensions);
    ...
    status = lw_scalar_average_pool2d_f32(
        tensor_input, pool_output, pool_input_dimensions, pool_output_dimensions,
        pool_kernel, pool_strides, pool_pads, 0u, 1u);
    ...
    status = lw_scalar_matmul_shared_f32(
        matmul_input, matmul_weights, matmul_output, 2u, 3u, 4u, 5u);
    ...
    status = lw_scalar_squeeze_f32(
        tensor_input, reshape_output, 4u, squeeze_input_dimensions,
        1u, invalid_squeeze_axis, 3u, NULL);
    ...
    status = lw_scalar_reduce_mean_f32(
        tensor_input, reduce_output, 3u, reduce_input_dimensions,
        2u, duplicate_reduce_axes, 1u, 0u, 3u, reduce_output_dimensions);
    if (!expect_status("reduce duplicate axis", status, LW_STATUS_INVALID_SHAPE)) {
        return 1;
    }
    status = lw_scalar_average_pool2d_f32(
        tensor_input, pool_output, pool_input_dimensions, invalid_pool_output,
        pool_kernel, pool_strides, pool_pads, 0u, 0u);
    ...
    status = lw_scalar_matmul_shared_f32(
        matmul_input, matmul_weights, matmul_input, 2u, 3u, 4u, 5u);
    if (!expect_status("matmul alias", status, LW_STATUS_INVALID_ARGUMENT)) {
        return 1;
    }
    if (is_32_bit_process()) {
        status = lw_scalar_matmul_shared_f32(
            matmul_input, matmul_weights, matmul_output,
            1u, 65536u, 65536u, 1u);
        if (!expect_status("matmul 32-bit byte overflow", status,
