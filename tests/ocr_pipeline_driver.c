...
static int run_crop(int argc, char** argv) {
    ...
    if (argc != 8 || !parse_u32(argv[3], &width) ||
        ...
        !parse_u32(argv[7], &vertical) || vertical > 1u ||
        !read_file(argv[2], &source, &source_bytes)) return 2;
    memset(&box, 0, sizeof(box));
    if (vertical == 0u) {
        box.x1 = 3.2f; box.y1 = 2.4f;
        box.x2 = 24.6f; box.y2 = 1.2f;
        box.x3 = 26.1f; box.y3 = 11.8f;
        box.x4 = 2.1f; box.y4 = 13.4f;
    } else {
        box.x1 = 8.0f; box.y1 = 1.0f;
        box.x2 = 14.0f; box.y2 = 2.0f;
        box.x3 = 12.0f; box.y3 = 16.0f;
        box.x4 = 6.0f; box.y4 = 15.0f;
    }
    status = lw_crop_quad_size(&box, &crop_width, &crop_height, &crop_bytes);
    if (status != LW_STATUS_OK || crop_bytes > SIZE_MAX) goto cleanup;
    crop = (uint8_t*)malloc((size_t)crop_bytes);
    if (crop == NULL) goto cleanup;
    status = lw_crop_quad_bgr_u8(
        source, source_bytes, width, height, stride, &box, crop, crop_bytes,
        &crop_width, &crop_height, &crop_bytes);
    if (status != LW_STATUS_OK ||
        !write_file(argv[6], crop, (size_t)crop_bytes)) goto cleanup;
...
static int run_pipeline(int argc, char** argv) {
    ...
    lw_status status;
    int return_code = 1;
    if (argc != 11 || !parse_u32(argv[7], &width) ||
        !parse_u32(argv[8], &height) || !parse_u32(argv[9], &stride) ||
        !parse_u32(argv[10], &use_classifier) || use_classifier > 1u ||
        !read_file(argv[6], &source, &source_bytes)) return 2;
    lw_ocr_options_init(&options);
    options.classifier.reserved = 1u;
    lw_error_init(&error);
    status = lw_ocr_create(argv[2], argv[3], argv[4], argv[5], &options,
                           &rejected, &error);
    ...
    lw_error_init(&error);
    status = lw_ocr_create(argv[2], NULL, argv[4], argv[5], &options,
                           &disabled, &error);
    if (status != LW_STATUS_OK || disabled == NULL) goto cleanup;
    ...
    options.detector.limit_side_length = 320u;
    lw_error_init(&error);
    status = lw_ocr_create(argv[2], use_classifier != 0u ? argv[3] : NULL,
                           argv[4], argv[5], &options, &ocr, &error);
    if (status != LW_STATUS_OK) {
    ...
    if (lw_ocr_get_info(ocr, &info) != LW_STATUS_OK ||
        info.use_direction_classification != use_classifier ||
        info.max_line_capacity == 0u || info.max_text_capacity == 0u)
        goto cleanup;
    memset(&query, 0, sizeof(query));
    lw_error_init(&error);
    status = lw_ocr_run_bgr_u8(
        ocr, source, source_bytes, width, height, stride,
        NULL, 0u, NULL, 0u, &query, &error);
    ...
    if (status != LW_STATUS_OK || query.line_count == 0u ||
        query.required_line_capacity != query.line_count ||
        query.required_text_capacity == 0u) {
    ...
    if (lines == NULL || text == NULL) goto cleanup;
    ...
    status = lw_ocr_run_bgr_u8(
        ocr, source, source_bytes, width, height, stride,
        lines, query.line_count - 1u, text, query.required_text_capacity,
        &result, &error);
    if (status != LW_STATUS_OUT_OF_BOUNDS ||
        result.required_line_capacity != query.line_count ||
        ((const unsigned char*)lines)[0] != 0x5au ||
        (unsigned char)text[0] != 0x5au) goto cleanup;
    lw_ocr_result_init(&result);
    lw_error_init(&error);
    status = lw_ocr_run_bgr_u8(
        ocr, source, source_bytes, width, height, stride,
        lines, query.line_count, text, query.required_text_capacity - 1u,
        &result, &error);
    if (status != LW_STATUS_OUT_OF_BOUNDS ||
    ...
    status = lw_ocr_run_bgr_u8(
        ocr, source, source_bytes, width, height, stride,
        lines, query.line_count, text, query.required_text_capacity,
        &result, &error);
    if (status != LW_STATUS_OK || result.line_count != query.line_count ||
        result.required_text_capacity != query.required_text_capacity)
        goto cleanup;
    printf("lines=%u detected=%u text_bytes=%llu resized=%ux%u cls=%u\n",
           result.line_count, result.detected_count,
           (unsigned long long)result.required_text_capacity,
    ...
    for (index = 0u; index < result.line_count; ++index) {
        ...
        if (line->text_offset + line->text_length >= result.required_text_capacity ||
            text[line->text_offset + line->text_length] != '\0') goto cleanup;
        printf("line=%u det=%.9g rec=%.9g cls_label=%u cls=%.9g rotation=%u "
               "point=%.9g,%.9g text=%s\n",
               index, (double)line->box.score,
               (double)line->recognition_score, line->classification_label,
               (double)line->classification_score,
               line->applied_rotation_degrees, (double)line->box.x1,
               (double)line->box.y1, text + line->text_offset);
    }
    lw_ocr_options_init(&options);
    options.use_direction_classification = 0u;
    options.detector.limit_side_length = 320u;
...
int main(int argc, char** argv) {
    if (argc < 2) return 2;
    if (strcmp(argv[1], "crop") == 0) return run_crop(argc, argv);
