...
static const float* tensor_input_data(
    ...
    if ((tensor->flags & LWM_V0_TENSOR_FLAG_CONSTANT) != 0u) {
        const uint8_t* disk = model->bytes + (size_t)model->tensor_offset +
...
static lw_status dispatch_node(
    lw_session* session,
    const uint8_t* node,
    uint32_t graph_input_index,
    const float* graph_input) {
    ...
    switch (op) {
        ...
        case LW_OP_MUL:
        case LW_OP_DIV: {
            lw_scalar_binary_op operation = op == LW_OP_ADD ? LW_SCALAR_BINARY_ADD :
                (op == LW_OP_MUL ? LW_SCALAR_BINARY_MUL : LW_SCALAR_BINARY_DIV);
            if (input_count != 2u) {
                return LW_STATUS_INVALID_SHAPE;
            }
            return lw_scalar_binary_f32(
                operation, inputs[0], input_tensors[0]->rank, input_tensors[0]->dimensions,
                inputs[1], input_tensors[1]->rank, input_tensors[1]->dimensions,
                output, output_tensor->rank, output_tensor->dimensions);
        }
        case LW_OP_ERF:
        ...
        case LW_OP_REDUCE_MEAN: {
            ...
            return lw_scalar_reduce_mean_f32(
                inputs[0], output, input_tensors[0]->rank, input_tensors[0]->dimensions,
                axes_count, axes, lwm_read_u32(params + 4), lwm_read_u32(params + 8),
        ...
        case LW_OP_AVERAGE_POOL: {
            ...
            return lw_scalar_average_pool2d_f32(
                inputs[0], output, input_tensors[0]->dimensions, output_tensor->dimensions,
                kernel, strides, pads, lwm_read_u32(params + 40), lwm_read_u32(params + 44));
        }
        case LW_OP_SQUEEZE:
        ...
        case LW_OP_UNSQUEEZE: {
            ...
            if (op == LW_OP_SQUEEZE) {
                return lw_scalar_squeeze_f32(
                    inputs[0], output, input_tensors[0]->rank, input_tensors[0]->dimensions,
                    axes_count, axes, output_tensor->rank, output_tensor->dimensions);
            }
            if (op == LW_OP_UNSQUEEZE) {
                return lw_scalar_unsqueeze_f32(
                    inputs[0], output, input_tensors[0]->rank, input_tensors[0]->dimensions,
                    axes_count, axes, output_tensor->rank, output_tensor->dimensions);
            }
            return lw_scalar_transpose_f32(
                inputs[0], output, input_tensors[0]->rank, input_tensors[0]->dimensions,
                axes_count, axes, output_tensor->dimensions);
        }
        case LW_OP_MATMUL: {
...
static lw_status execute_session_f32(
    ...
    if (model->info.input_count != 1u || model->info.output_count != 1u) {
        lw_set_error(error, LW_STATUS_UNSUPPORTED, "private executor currently requires one input and one output");
        return LW_STATUS_UNSUPPORTED;
    }
    ...
    if (session->tensors[graph_input_index].dtype != LW_DTYPE_F32 ||
        ...
        lw_set_error(error, LW_STATUS_INVALID_SHAPE, "input or output element count does not match the session");
        return LW_STATUS_INVALID_SHAPE;
    }
    for (node_index = 0u; node_index < model->info.node_count; ++node_index) {
        ...
        if (status != LW_STATUS_OK) {
            ...
            (void)sprintf_s(message, sizeof(message),
                "node %u (operator %u) failed: %s", node_index,
