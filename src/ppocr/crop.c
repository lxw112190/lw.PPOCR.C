...
static int load_points(const lw_detection_box* box, crop_point points[4]) {
    ...
    for (index = 0u; index < 4u; ++index) {
        if (!isfinite(values[index * 2u]) ||
            !isfinite(values[index * 2u + 1u])) return 0;
...
lw_status lw_crop_quad_size(
    ...
    uint64_t height;
    int rotate_vertical;
    if (crop_width != NULL) *crop_width = 0u;
    if (crop_height != NULL) *crop_height = 0u;
    if (crop_byte_count != NULL) *crop_byte_count = 0u;
    if (box == NULL || crop_width == NULL || crop_height == NULL ||
        crop_byte_count == NULL || !load_points(box, points))
    ...
    if (!isfinite(width_value) || !isfinite(height_value) ||
        ...
        width_value > UINT32_MAX || height_value > UINT32_MAX)
...
static uint8_t sample_channel(
    ...
    double wx = x - x0_raw;
    double wy = y - y0_raw;
    double top_left = source[(size_t)((uint64_t)y0 * source_stride +
        (uint64_t)x0 * 3u + channel)];
    double top_right = source[(size_t)((uint64_t)y0 * source_stride +
        (uint64_t)x1 * 3u + channel)];
    double bottom_left = source[(size_t)((uint64_t)y1 * source_stride +
        (uint64_t)x0 * 3u + channel)];
    double bottom_right = source[(size_t)((uint64_t)y1 * source_stride +
        (uint64_t)x1 * 3u + channel)];
...
lw_status lw_crop_quad_bgr_u8(
    ...
    status = lw_crop_quad_size(
        box, &output_width, &output_height, &required_bytes);
    if (status != LW_STATUS_OK) return status;
    if (source == NULL || crop == NULL || source_width == 0u ||
        source_height == 0u || !load_points(box, points))
        return LW_STATUS_INVALID_ARGUMENT;
    ...
    required_source_bytes =
        (uint64_t)(source_height - 1u) * source_stride + row_bytes;
    if (source_byte_count < required_source_bytes ||
        required_source_bytes > SIZE_MAX)
        return LW_STATUS_INVALID_SHAPE;
    if (crop_capacity < required_bytes || required_bytes > SIZE_MAX)
        return LW_STATUS_OUT_OF_BOUNDS;
    unrotated_width = (uint32_t)floor(
        point_distance(points[0], points[1]) + 0.5);
    unrotated_height = (uint32_t)floor(
        point_distance(points[0], points[3]) + 0.5);
    rotate_vertical =
        (double)unrotated_height >= (double)unrotated_width * 1.5;
    ...
    for (y = 0u; y < unrotated_height; ++y) {
        ...
        for (x = 0u; x < unrotated_width; ++x) {
            ...
            double source_x;
            double source_y;
            uint32_t destination_x = rotate_vertical ?
                unrotated_height - 1u - y : x;
            uint32_t destination_y = rotate_vertical ? x : y;
            uint64_t destination =
                ((uint64_t)destination_y * output_width + destination_x) * 3u;
            uint32_t channel;
            if (!isfinite(denominator) || fabs(denominator) <= 1.0e-12)
                return LW_STATUS_INVALID_SHAPE;
            ...
            for (channel = 0u; channel < 3u; ++channel) {
                crop[(size_t)(destination + channel)] = sample_channel(
...
void lw_rotate_bgr_u8_180(
    ...
    for (left = 0u; left < pixel_count / 2u; ++left) {
        ...
        for (channel = 0u; channel < 3u; ++channel) {
            ...
            pixels[(size_t)(left * 3u + channel)] =
                pixels[(size_t)(right * 3u + channel)];
